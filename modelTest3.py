...
# ジェネレーターのインスタンスを作成
train_gen = DataGenerator(train_image_dir, train_mask_dir, batch_size=16)
val_gen = DataGenerator(val_image_dir, val_mask_dir, batch_size=16)
...
model = Sequential([
    ...
    Conv2DTranspose(32, (3, 3), strides=(2, 2), padding='same', activation='relu'),   # 480x480 -> 960x960
    Conv2D(1, (1, 1), activation='sigmoid')  # 最後に1チャネルの出力を得る
])
