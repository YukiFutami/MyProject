...
def process_images_in_folder(folder_path):
    for image_filename in os.listdir(folder_path):
        ...
        # 予測結果に基づく車両制御
        control_vehicle(white_line_prediction, direction_prediction, image_filename)

# 車両制御
def control_vehicle(white_line_prediction, direction_prediction, image_filename):
    # 出力に基づく制御の例
    if direction_prediction == "straight":
        set_servo_angle(90)  # 中央
        result_message = f"Image: {image_filename}, Direction: Straight"
    elif direction_prediction == "left":
        set_servo_angle(120)  # 左方向
        result_message = f"Image: {image_filename}, Direction: Left"
    elif direction_prediction == "right":
