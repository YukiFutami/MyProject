...
class DataGenerator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        for img_name, mask_name in zip(batch_images, batch_masks):
            ...
            img = cv2.resize(img, self.target_size)  # 256x256にリサイズ
            
            # マスクの読み込みと正規化
            mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE) / 255.0  # 正規化
            mask = np.round(mask)  # 0または1に正規化
            mask = cv2.resize(mask, self.target_size)  # 256x256にリサイズ
            mask = np.expand_dims(mask, axis=-1)  # チャンネル次元を追加（1チャネル）
...
model = Sequential([
    ...
    Conv2DTranspose(128, (3, 3), strides=(2, 2), padding='same', activation='relu'),  # 出力サイズ確認
    Conv2DTranspose(64, (3, 3), strides=(2, 2), padding='same', activation='relu'),   # 出力サイズ確認
    
    # 最終的に256x256に縮小
    Conv2DTranspose(1, (3, 3), strides=(2, 2), padding='same', activation='sigmoid')  # 出力サイズ確認
])

# モデルの構築後にsummary()で出力サイズを確認
model.summary()
